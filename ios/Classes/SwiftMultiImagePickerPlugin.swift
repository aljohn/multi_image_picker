                    ...
                    resultHandler: {
                        (image: UIImage?, info) in
                         if let originalImage = image {
                             self.messenger.send(onChannel: "multi_image_picker/image/" + identifier + ".original", message: originalImage.jpegData(compressionQuality: CGFloat(compressionQuality)))
                         } else {
                             print("\(info)");
                         }
                })
